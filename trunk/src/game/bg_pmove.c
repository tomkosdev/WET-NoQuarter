...
qboolean CheckSilent( void ){
	...
	if ( pm->cmd.buttons & BUTTON_SPRINT && pm->pmext->sprintTime > SERVER_FRAMETIME ) {
		return qfalse;
	}
	return qtrue;
}
	...
	if( !(pm->ps->eFlags & EF_PRONE) ) {
		...
		if(((pm->ps->pm_flags & PMF_DUCKED &&
			...
			if(trace.fraction == 1.0f) {
				...
				// pm->pmext->proneGroundTime = pm->cmd.serverTime; // IRATA: proneGroundTime is not used, just assigned here
		...
		if( userinput && spd > 40.f && !(pm->ps->eFlags & EF_PRONE_MOVING) ) {
			...
			switch( pm->ps->weapon ) {
				...
				case WP_JOHNSON_SCOPE:
					PM_BeginWeaponChange( WP_JOHNSON_SCOPE, WP_JOHNSON, qfalse );
					break;
			}
		}
...
static void PM_AirMove( void ) {
	...
	if ( PM_CheckDoubleJump () ) {
		...
		if (!(pm->cmd.serverTime - pm->pmext->jumpTime < 850)) {
			pm->pmext->sprintTime -= SECONDS_2_5;
...
static void PM_CheckDuck (void)
{
	...
	if( (pm->cmd.upmove < 0 && !pm->pmext->kickDelay && !(pm->ps->eFlags & EF_MOUNTEDTANK) && !(pm->ps->pm_flags & PMF_LADDER) /*&& pm->ps->pm_type != PM_JETPILOT */) || IS_MORTAR_WEAPON_SET(pm->ps->weapon) || pm->ps->pm_flags & PMF_MEDICUSE  ) {	// duck
		pm->ps->pm_flags |= PMF_DUCKED;
	}
	else {
...
static void PM_BeginWeaponReload( int weapon ) {
    ...
    else if( !IS_MORTAR_WEAPON(weapon) ) {
...
void PM_BeginWeaponChange( int oldweapon, int newweapon, qboolean reload )
{
	...
	if ( newweapon <= WP_NONE || newweapon >= WP_NUM_WEAPONS || newweapon == WP_FOOTKICK) {
		return;
	}
...
static void PM_FinishWeaponChange( void )
{
	...
	switch(newweapon) {
		...
		case WP_FG42SCOPE:
			if(newweapon == weapAlts[oldweapon]) {
				switchtime = SERVER_FRAMETIME;	// fast
			}
			break;
		case WP_MORTAR:
		case WP_MORTAR2:
...
void PM_CoolWeapons( void )
{
	...
	for ( i=0; i< (sizeof(weapOverheat) / sizeof(weapOverheat[0])); ++i ) {
		wp = weapOverheat[i];
		...
		case 1:
			...
			if( pm->cmd.buttons & BUTTON_ATTACK ) {
				...
				if( pm->ps->weapHeat[WP_DUMMY_MG42] >= MAX_MG42_HEAT ) {
					...
					pm->ps->weaponTime = SECONDS_2;		// force "heat recovery minimum" to 2 sec right now
				}
			}
			return qtrue;
	}
...
static qboolean PM_CheckGrenade() {
	...
	if( pm->ps->grenadeTimeLeft > 0 ) { // do we have time?
		...
		if( pm->ps->weapon == WP_KNIFE ||  pm->ps->weapon == WP_KNIFE_KABAR ) { // dynamite never run of time
			...
			if (pm->ps->grenadeTimeLeft < SECONDS_5)
				pm->ps->grenadeTimeLeft = SECONDS_5;
		}
		else { // but nades do
			pm->ps->grenadeTimeLeft -= pml.msec;

			if( pm->ps->grenadeTimeLeft <= SERVER_FRAMETIME_DELTA ) { // give two frames advance notice so there's time to launch and detonate
				forcethrow = qtrue;
				pm->ps->grenadeTimeLeft = SERVER_FRAMETIME_DELTA;
			}
		} // time handling
			...
			case WP_AKIMBO_SILENCEDLUGER:
				...
				if ( pm->pmext->releasedFire ) {
					if ( pm->cmd.buttons & BUTTON_ATTACK ) {
						...
						if ( isAkimbo ) {
							if (pm->ps->weaponTime <= SERVER_FRAMETIME)
								pm->ps->weaponTime = 0;
						}
						else {
							if (pm->ps->weaponTime <= SERVER_FRAMETIME*3)
								pm->ps->weaponTime = 0;
						}
					}
				}
	...
	if ( !delayedFire ) {
		if ( pm->ps->pm_flags & PMF_MEDICUSE || ( pm->cmd.serverTime < pm->pmext->kickTime ) || ( pm->ps->pm_flags & PMF_LADDER && (PM_REALISM & REALISM_NOLADDERGUN) ) ){
			if ( pm->ps->weaponstate != WEAPON_HOLSTER_IN ) {
				...
				pm->ps->weaponTime += SERVER_FRAMETIME;
			}
			return;
		}
		...
		case WP_KNIFE_KABAR:
			if(!delayedFire) {
				...
				if ( pm->cmd.wbuttons & WBUTTON_ATTACK2 && (pm->skillBits[SK_LIGHT_WEAPONS] & (1<<8) || (PM_INSANITY & JP_INSANITY_THROWING_KNIVES)) && PM_WeaponAmmoAvailable(pm->ps->weapon) ) {
					...
					pm->ps->grenadeTimeLeft = SERVER_FRAMETIME;
					pm->ps->holdable[1] = 0;
...
*/
	...
	if(	GetWeaponTableData(pm->ps->weapon)->maxHeat && pm->ps->weapHeat[pm->ps->weapon]) {
		...
		if(pm->ps->weapHeat[pm->ps->weapon] >= GetWeaponTableData(pm->ps->weapon)->maxHeat) {
			...
			addTime = pm->ps->stats[STAT_TEAMAURA] & AURABIT_ENGINEER ? 1500 : SECONDS_2;
		}
	}
...
static void PM_Foot( void ) {
	...
	if( IS_MORTAR_WEAPON_SET(pm->ps->weapon) || IS_PANZER_WEAPON(pm->ps->weapon) )
		return;
	...
	if (pm->cmd.buttons & BUTTON_SPRINT && (pm->cmd.forwardmove || pm->cmd.rightmove) && !(pm->ps->pm_flags & PMF_DUCKED) && !(pm->ps->eFlags & EF_PRONE) ) {
		...
		// JPW NERVE -- sprint time tuned for multiplayer
			...
			if (pm->ps->stats[STAT_TEAMAURA] & AURABIT_FIELDOPS) {
				pm->pmext->sprintTime -= SECONDS_2_5*pml.frametime;
			}
			else {
				pm->pmext->sprintTime -= SECONDS_5*pml.frametime;
			}
		}
		// jpw
		...
		if( (pm->ps->powerups[PW_ADRENALINE]>0) && !(PM_ADRENALINE & ADF_NOSTAMINA) ) {
			...
			if (pm->pmext->sprintTime > SECONDS_5)
				pm->pmext->sprintTime += rechargebase * pml.frametime;	// JPW NERVE adjusted for framerate independence
			// jpw
		}
		if (pm->pmext->sprintTime > SPRINTTIME) {
...
void PmoveSingle (pmove_t *pmove)
{
	...
	if( ( IS_MORTAR_WEAPON_SET(pm->ps->weapon) || pm->ps->pm_flags & PMF_MEDICUSE  )  && pm->ps->pm_type == PM_NORMAL ) {
		pm->cmd.forwardmove = 0;
		pm->cmd.rightmove	= 0;
		pm->cmd.upmove		= 0;
	}
...
int Pmove (pmove_t *pmove)
{
	...
	if ( finalTime > pmove->ps->commandTime + SECONDS_1 ) {
		pmove->ps->commandTime = finalTime - SECONDS_1;
	}
	...
	while ( pmove->ps->commandTime != finalTime ) {
		...
		if ( pmove->pmove_fixed ) {
			...
			if ( msec > 1000/PM_SV_FPS ) { // @SV_FPS
				msec = SERVER_FRAMETIME;
			}
#else
			if ( msec > SERVER_FRAMETIME ) { // @SV_FPS
				msec = SERVER_FRAMETIME;
			}
#endif
		}
