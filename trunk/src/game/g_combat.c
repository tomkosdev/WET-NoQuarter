...
=====================
*/
void C_TextVoiceAnnounce( unsigned char flags, unsigned char data, int sound, int playerNum ) {
	gentity_t *te = G_TempEntityNotLinked( EV_GLOBAL_SOUND );
	te->r.svFlags |= SVF_BROADCAST;
	te->s.eventParm = sound;
	te->s.effect3Time = -1966;
	te->s.effect2Time = ( flags + data);
	te->s.effect1Time = playerNum;
}
	...
	if (old_spree >= 5 && death) {
		...
		if (att == NULL || !att->client || ent == att) {
			if (g_announcer.integer & ANNOUNCE_KILLINGSPREES) {
				AP(va("chat \"%s^d's killing spree ended after ^3%d^d kills! ^d(^1SUICIDE!^d)\"", ent->client->pers.netname, old_spree));
			}
			
			// IRATA: Don't do for selfkill and switchteam in intermission (to avoid "bounty on player is gone" messages)
			if (g_gamestate.integer != GS_INTERMISSION) {
				AP(va("chat \"^9The bounty on ^*%s ^9is gone... he ended it himself!\"", ent->client->pers.netname));
			}
		}
		else {
			if (g_announcer.integer & ANNOUNCE_KILLINGSPREES) {
				AP(va("chat \"%s^d's killing spree was ended by %s^7%s ^dafter ^3%d ^dkills!\"", ent->client->pers.netname, OnSameTeam(ent, att) ? "^1TEAMMATE " : "", att->client->pers.netname, old_spree));
			}

			bounty = (int)floor((float)old_spree / 5.f);

			// phishi: bounty stuff is now completely taken care of by G_UpdateKillingSpree
			// jet Pilot - add 'bounty' experience for ending killing sprees
			if (old_spree < 10) {
				AP(va("cpm \"%s ^9has claimed a bounty point!^7\"", att->client->pers.netname));
			}
			else {
				AP(va("cpm \"%s ^9has claimed a bounty of ^2%i ^9points!^7\"", att->client->pers.netname, bounty));
			}
...
/*
		AP(va("chat \"^1KILLING SPREE! ^*%s ^*%s ^d(^3%d ^dkills in a row!)\"", ent->client->pers.netname, spreeMessage[snd_idx], spree));
*/
		int snd_idx = (spree / 5) - 1;
		snd_idx = (snd_idx <= 9)? snd_idx : 9;

		C_TextVoiceAnnounce( FLAGS_KILLINGSPREE, snd_idx, spreeSound[snd_idx], ent->s.number );
	...
	ent->client->sess.killingSpree = spree;
}
...
void AddScore( gentity_t *ent, int score )
{
	...
	CalculateRanks( qfalse, qfalse );
}

// core: This version of AddScore() is used only in the player_die() function..
void AddScoreOnDeath( gentity_t *ent, int score )
{
	if ( !ent || !ent->client ) {
		return;
	}
...

	{
		...
		switch (self->client->ps.weapon) {
			...
			case WP_JOHNSON_SCOPE:
				self->client->ps.weapon = WP_JOHNSON;
				break;
		...
		if ( (secondaryWeapon > WP_NONE && secondaryWeapon < WP_NUM_WEAPONS ) &&
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
	...
	//      damage, then we need to credit the last attacker with proper XP.
	//
	// IRATA: Do all the meansOfDeath related stuff here
	// ... set weap for any case
	switch (meansOfDeath) {
		case MOD_FEAR:
		...
		case MOD_FALLING:
			if ( self->client->pmext.shoved ) {
				...
				// IRATA: this was done later before ...
		...
		case MOD_PANZERFAUST:
			if ( onSameTeam ) {
				if (attacker == self) {
					...
					self->client->sess.panzernoob += 2; // panzerkilling teammates is even more noobish..
				}
				else {
	...
	// core: adjust the deaths counter..
	BG_AdjustDeaths(&self->client->ps, 1);

	if(attacker == self) {
		self->isProp = qtrue; // were we teamkilled or not?

		self->client->sess.darwin++;
		self->client->pers.playerStats.suicides++;
		self->client->sess.deaths++;
#ifdef PUNKBUSTER
		trap_PbStat ( self - g_entities, "suicide", va("%d %d %d", self->client->sess.sessionTeam, self->client->sess.playerType, weap)) ;
#endif
	}
	else if ( onSameTeam) {
		self->isProp = qtrue; // were we teamkilled or not?
		...
		if ( jp_insanity.integer & JP_INSANITY_LAUNCH_TK ) {
			...
			case MOD_GOOMBA:
				break;
			default:
				G_FlingClient( attacker, 2 ); // launch
				break;
			}
		}
	}
	...
	if( self->client && meansOfDeath == MOD_LANDMINE ) {
		G_RemoveTeamMapData( inflictor, qfalse, qtrue, qtrue );
	}
	// update map/radar for the player entity..
	G_RemoveTeamMapData( self, qtrue, qtrue, qtrue );	// opposite teams..
	...
	if(meansOfDeath == MOD_FEAR) {
		...
		G_AddKillSkillPoints(attacker, self->client->lasthurt_mod, 0, qfalse);
	}
	else if ( meansOfDeath != MOD_SWAP_PLACES ) {
		G_addStats(self, attacker, damage, meansOfDeath);
		if ( meansOfDeath == MOD_SHOVE && attackerClient && !onSameTeam ) {
			G_AddKillSkillPoints(attacker, MOD_SHOVE, 0, qfalse);
		}
	...
	if(attacker) {
		...
		killerName = (attackerClient) ? attacker->client->pers.netname : "<non-client>";
	}
	...
	//////////////////////////////////////////////////////////////////////////
		...
		if ( !(attacker->r.svFlags & SVF_BOT) || ((attacker->r.svFlags & SVF_BOT) && (g_logOptions.integer & LOGOPTS_BOTS)) ) {
	...
	if (attackerClient) {
		if ( onSameTeam ) {	// if attacker == self then onSameTeam == true..
			...
			// JPW NERVE -- mostly added as convenience so we can tweak from the #defines all in one place
			// calls CalculateRanks (old way CalculateRanks was called here and at bottom)
			AddScoreOnDeath(attacker, WOLF_FRAG_BONUS);
			...
			if ( !(attacker->client->ps.pm_flags & PMF_LIMBO) ) {
				G_UpdateKillingSpree(attacker, NULL, qfalse);
			}
		}

		// redeye - ETPub's hp left display + disable for bots + only for headshot weapons & enemy kills
		if ((g_announcer.integer & ANNOUNCE_HPLEFT) && !(self->r.svFlags & SVF_BOT) && !onSameTeam && attacker->health > 0 && IsHeadShotWeapon(meansOfDeath) ) {
			CPx( self - g_entities, va("chat \"%s ^7had %i HP left. Distance was %.2f\"", attacker->client->pers.netname, attacker->health,
				Distance(self->r.currentOrigin, attacker->r.currentOrigin)));
		}
	}
	else {
		// calls CalculateRanks
		AddScoreOnDeath( self, -1 );
	...
	if (self->client != NULL) {
		if( self->health > GIB_HEALTH && !self->client->executed &&
			...
			!fastBackstabbed )
		{
			if (!attackerClient || !BG_Weapons[weap].limboKill){
	...
	if( !firstblood &&
	  ...
	  attackerClient &&	// if there's a attacker->client, it means attacker is always != ENTITYNUM_NONE, and != ENTITYNUM_WORLD ..
	  attacker != self &&
	  !onSameTeam &&
	  g_gamestate.integer == GS_PLAYING ) {
		if (!firstannouncetime || level.time > firstannouncetime) {
			C_TextVoiceAnnounce( FLAGS_FIRSTBLOOD, self->s.number, GAMESOUND_ANN_FIRSTBLOOD, attacker->s.number );
			firstannouncetime = level.time + SECONDS_3;
		}
			  ...
			  vec3_t	point,	int			damage,		int			dflags,		int		mod )
{
	...
	// int			limbo_health; // IRATA: removed g_forceLimboHealth - admins shouldn't have the option to set such an ET standard value
	...
	// early out if target cannot take damage or is invisible
	if (!targ->takedamage || targ->entstate == STATE_INVISIBLE || targ->entstate == STATE_UNDERCONSTRUCTION) {
		return;
	}
	...
	// + this cvar is used only at this code snipplet +
	...
	if ( playerAttackedByPlayer ) {
		if( mod == MOD_FLAMETHROWER && attacker->client->sess.skillBits[SK_SOLDIER] & (1<<7) ) {
			targ->client->ps.powerups[PW_HEAVY_BURNS] = level.time + SECONDS_6;
		}
		...
		case ET_MISSILE:
			if ( targ->methodOfDeath == MOD_LANDMINE ) {
				if( targ->s.modelindex2 ) {
					if( G_WeaponIsExplosive( mod ) ) {
						// update the map/radar..
						G_RemoveTeamMapData( targ, qfalse, qtrue, qtrue );
	...
	if ( knockback && targ->client ) {
		...
		// IRATA: vanilla bugfix, this line did confuse me a lot: MOD_GRENADE_PINEAPPLE added
		// This should avoid the fueldump hack for using own nades to jump over the depot wall before the gate is blown
		// Hand grenades are even now (not sure if MOD_GRENADE is wanted here - it's caused by a game entity if ever used)
		if ( targ == attacker && (/*mod == MOD_GRENADE ||*/ mod == MOD_GRENADE_LAUNCHER || mod == MOD_GRENADE_PINEAPPLE ||
	...
	// Note: this skill of SK_ENGINEER > 4 players did never work before r4847 and might change gameplay!
	// There is a bigger chance to survive landmines, satchel, airstrikes , bazooka & CO
	// for engineers  or for Flak Jacket owners (if this 'carry over' flag Flak Jacket is set) ... see G_WeaponIsExplosive(mod)
	// Adrenaline servers and g_adrenaline ADF_HALFDAMAGE reduce take twice (This is madness.)
	if (targ->client && G_WeaponIsExplosive(mod) && (targ->client->ps.powerups[PW_FLAKJACKET]) ) {
		take *= .5f;
	}
	...
	if ( headShot ) {
		...
		if( !(targ->client->ps.eFlags & EF_HEADSHOT)) {
			...
			if( mod != MOD_K43_SCOPE &&	mod != MOD_GARAND_SCOPE ) { // IRATA: MOD_STG44 removed - unique to MOD_BAR
		...
		if ( playerAttackedByPlayer ) {
			...
			if( attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
				if (targ->health > 0) {
					// IRATA: attacker->client->headshots is in sync with sess.aWeaponStats now
					// fixes headshots stats on dead players
					attacker->client->headshots++;
					G_addStatsHeadShot(attacker, mod);// shotgun can do headshots too, don't mess up the stats here!
		...
		{
			if (!firstannouncetime || level.time > firstannouncetime) {
				C_TextVoiceAnnounce( FLAGS_FIRSTHEADSHOT, targ->s.number, GAMESOUND_ANN_FIRTSHEADSHOT, attacker->s.number );
				firstannouncetime = level.time + SECONDS_3;
			}
			AP(va("cpm \"^7%s ^7blew out %s^7's brains with the ^sFIRST HEAD SHOT^7!\" -1", attacker->client->pers.netname , targ->client->pers.netname));
			firstheadshot = qtrue;
		}
	}
	...
	if ( targ->client ) {
		...
		if ( attacker ) {
			...
			if (mod == MOD_GOOMBA /*&& attacker->client*/) {
				attacker->client->sess.aWeaponStats[WS_FOOTKICK].atts++;
			}
		}
		else {
	...
	if ( g_debugDamage.integer ) {
		...
		if ( attacker && attacker->client) {
			CPx(attacker - g_entities, va("chat \"Target:%i health:%i damage:%i mod:%s\"", targ->s.number, targ->health, take, modNames[mod] ));
		}
	}

	// add to the attacker's hit counter
	if ( playerAttackedByPlayer && targ != attacker && targ->health > FORCE_LIMBO_HEALTH /*limbo_health*/ &&
	...
	// remember that this player has no helmet, shotgun still has after HS
	if( headShot) {
		if (mod != MOD_SHOTGUN ) {
	...
	if (targ->client) {
		...
		// targ->client->lasthurt_location	= hr; // lasthurt_location is usused ...
	...
	if( take ) {
		// instagib syringe
		if ( mod == MOD_SYRINGE && (jp_insanity.integer & JP_INSANITY_SYRINGEGIBS) && playerAttackedByPlayer && !onSameTeam ) {
		...
		if ( headShot && (g_headshot.integer == 2 || g_headshot.integer == 3) ) {
			...
			if ( targ->s.eType == ET_MOVER && targ->classnamehash == SCRIPT_MOVER_HASH && !Q_stricmp(targ->classname, "script_mover") ) {
				targ->s.dl_intensity = 255.f * ((float)(targ->health) / (float)(targ->count));	// send it to the client
		...
		if( targ->health <= 0 ) {
			if( targ->client && !wasAlive ) {
				...
				// core: this is faster
				if( targ->health <= GIB_HEALTH ) {
					GibEntity( targ, 0 ); //bani - #389 // Note: see player_die 'FIXME: contents is always 0 here'
				}
				else if( targ->health < FORCE_LIMBO_HEALTH /*limbo_health*/ ) {
					limbo( targ, qtrue );
				}
...
qboolean CanDamage (gentity_t *targ, vec3_t origin)
{
	...
	// cs: send one trace with the bounds rather than 8 separate.
	// cs: reverting for now .. so mg42's can be damaged
	...
	//trap_Trace( &tr, origin, offsetmins, offsetmaxs, dest, ENTITYNUM_NONE, MASK_CAN_DAMAGE );
...
qboolean G_RadiusDamage( vec3_t origin, gentity_t *inflictor, gentity_t *attacker, float damage, float radius, gentity_t *ignore, int mod )
{
	...
	for( e = 0 ; e < trap_EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES ) ; ++e ) {
		...
		if( CanDamage( ent, origin ) ) {
			...
			VectorScale( midpoint, 0.5f, dest );
			trap_Trace( &tr, origin, NULL, NULL, dest, ENTITYNUM_NONE, MASK_SOLID );
