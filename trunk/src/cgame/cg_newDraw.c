...
static int weapIconDrawSize(int weap) {
	...
	switch(weap) {
		...
		case WP_MP34: // - jaquboss was missing
		case WP_THOMPSON:
		case WP_MP40:
		case WP_JOHNSON:
...
void CG_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor )
{
	...
	if( cg.predictedPlayerEntity.currentState.eFlags & EF_MG42_ACTIVE || cg.predictedPlayerEntity.currentState.eFlags & EF_MOUNTEDTANK )
		// jet Pilot ??? something with the browning here?
	...
	if(cg.predictedPlayerState.grenadeTimeLeft) {	// grenades and dynamite set this
		...
		if ( realweap ==  WP_KNIFE || realweap == WP_KNIFE_KABAR ) {
			scale = (float)(cg.predictedPlayerState.grenadeTimeLeft/(KNIFECHARGETIME/10.f));
			halfScale = scale * 0.5f;
		}
		else if( realweap == WP_DYNAMITE ) {
			if(((cg.grenLastTime)%SECONDS_1)>((cg.predictedPlayerState.grenadeTimeLeft)%SECONDS_1)) {
				trap_S_StartLocalSound(cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND );
			}
		}
		else {
			if(((cg.grenLastTime)%SECONDS_1)<((cg.predictedPlayerState.grenadeTimeLeft)%SECONDS_1)) {
				switch( cg.predictedPlayerState.grenadeTimeLeft/SECONDS_1) {
					case 3:
						trap_S_StartLocalSound(cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND );
						break;
					case 2:
		...
		if ( realweap != WP_KNIFE && realweap != WP_KNIFE_KABAR ) {
			scale = (float)((cg.predictedPlayerState.grenadeTimeLeft)%SECONDS_1) / 100.0f;
			halfScale = scale * 0.5f;
		}
...
void CG_DrawCursorhint(rectDef_t *rect) {
	...
	switch(cg.cursorHintIcon) {
		...
		case HINT_KNIFE:
			...
			break;
		case HINT_PISTOL:
			icon = cgs.media.pistolHintShader[0];
			break;
		case HINT_LUGER:
			icon = cgs.media.pistolHintShader[1];
			break;
		case HINT_LADDER:
		...
		case HINT_DISARM:
			...
			break;
		case HINT_REVIVE_ACTIVATE:
			icon = cgs.media.reviveActivateHintShader;
			break;
		case HINT_REVIVE:
	...
	if(cg_cursorHints.integer >= 3) {	// no size pulsing
		...
		if(cg_cursorHints.integer == 2)
			scale = (float)((cg.cursorHintTime)%SECONDS_1) / 100.0f;	// one way size pulse
		else
	...
	if (cg.cursorHintIcon == HINT_WEAPON || cg.cursorHintIcon == HINT_AMMO ){
		if ( cg_weapons[ cg.cursorHintValue ].weaponIcon[1] )
		CG_DrawPic( middle - (weaponIconScale(cg.cursorHintValue) * halfscale), rect->y - halfscale,  weaponIconScale(cg.cursorHintValue) * (rect->w + scale), rect->h + scale,  cg_weapons[ cg.cursorHintValue ].weaponIcon[1] );
		if ( cg.cursorHintIcon == HINT_AMMO ) // draw ammo over it
...
void CG_DrawWeapStability( rectDef_t *rect ) {
	...
	if( cg_drawSpreadScale.integer == 1 && !IS_SCOPED_WEAPON( cg.predictedPlayerState.weapon ) ) {
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	// IRATA: content useless so removed
}
...
void CG_MouseEvent(int x, int y) {
	switch( cgs.eventHandling ) {
		...
		case CGAME_EVENT_DEMO:
			...
			if( x != 0 || y != 0 ) {
				cgs.cursorUpdate = cg.time + SECONDS_5;
			}
			break;
...
void CG_EventHandling( int type, qboolean fForced )
{
	...
	switch( type ) {
		...
		case CGAME_EVENT_DEMO:
			cgs.fResize = qfalse;
			cgs.fSelect = qfalse;
			cgs.cursorUpdate = cg.time + SECONDS_10;
			cgs.timescaleUpdate = cg.time + SECONDS_4;
			CG_ScoresUp_f();
			break;
...
int CG_ClientNumFromName(const char *p) {
	...
	for( i = 0; i < cgs.numValidClients; ++i ) {
		...
		if (Q_stricmp(cgs.clientinfo[clientNum].name, p) == 0) {
			return clientNum;
		}
	}
	return -1;
