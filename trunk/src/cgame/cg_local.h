...
#define DAMAGE_TIME				SECOND_HALF
...
#define	SINK_TIME				SECONDS_1	// time for fragments to sink into ground before going away
#define	REWARD_TIME				3000

#define PRONE_TIME				SECOND_HALF
...
typedef struct playerEntity_s
{
	...
	lerpFrame_t		hudhead;
	lerpFrame_t		foot;			// 1st person foot
	...
	refEntity_t		bodyRefEnt;
	refEntity_t		headRefEnt;
	refEntity_t		gunRefEnt;
	...
	// jet Pilot - visible leaning from FalkonET
	int				leanTime;
	float			leanDirection;
	int				leanDir;
	// jaquboss - brass origin
	vec3_t			ejectBrassCasingOrigin;

	// jaquboss - thirdperson head animations
	int				nextIdleTime;
	int				lastIdleTimeEnd;
	int				idleAnim;
	qboolean		respawning;
...
typedef struct clientInfo_s
{
	...
	// Gordon: Intermission stats // IRATA: move this out of clientInfo_t (only intermission!)
	int				totalWeapAcc;
	int				kills;
	int				deaths;
	int				team_kills_i; // IRATA
...
typedef struct partModel_s {
	char		tagName[MAX_QPATH];
	qhandle_t	model[3];				// 0: neutral, 1: axis, 2: allied
...
typedef struct weaponInfo_s {
	...
	sfxHandle_t		deathBySound[4];
	sfxHandle_t		deathBySoundFar[4];

	int				deathBySoundNum;
	int				deathBySoundFarNum;

	qhandle_t		weaponIcon[2];		//----(SA)	[0] is weap icon, [1] is highlight icon
	qhandle_t		ammoIcon;

	qhandle_t		missileModel[3];	// cs: changed from [2]. calls like missileModel[TEAM_ALLIES] were out of bounds!
	...
	// jaquboss other data to supply
	//screen shake
	int				recoil_pitchAdd;
	int				recoil_yawRandom;
	int				recoil_pitchAddRandom;

	// barrel smoke
	qhandle_t		flashSmokeShader;
	float			flashSmokeSize;
	int				flashSmokeLife;
	float			flashSmokeAlpha;

	// ejecting brass
	qhandle_t		brassModel;

	// @todo move all models variations here to aware all errors
	qboolean		alliedMissileModel;
	qboolean		axiesMissileModel;
	int				weaponIconScale;
...
void CG_ParseMapEntityInfo2( int axis_number, int allied_number );
...
typedef struct cg_s
{
	...
	qboolean	duckPlaydead;			// sigh, since playdead is not predicted we need to know this
	...
	// jet Pilot - leaning time - from falkonet
	int			leanTime;
	...
	// jP - announcement printing
	int			announcementPrintTime;
	int			announcementCharWidth;
	int			announcementPrintY;
	int			announcementPrintLines;
	...
	int			lastIdleTimeEnd;
	int			idleAnim;
	int			lastUsedWeaponBank;	// core: for choosing next weapon, after using cretain weapons (smoke,panzer,airstrike etc)
...
// JPW FIXME NOTE: max_weap_banks > max_weap_banks_mp so this should be OK, but if that changes, change this too
	...
	qboolean		editingSpeakers;		// using the speaker editor GUI
	...
	forceCvar_t			cvarBackups[MAX_FORCECVARS+MAX_SVCVARS];
	...
	qboolean		subscene;
	// core - KILLERCAM
	centity_t		*latestKiller;
	int				killerCamTime;


	// jaquboss bloody knifes
	int				knifeBloodVal; // IRATA: we don't use it

	// jaquboss - tracing bullet, predict hitboxes used on server
	qboolean		bulletTrace;

	//				jaquboss, centerPrint with effect
	...
	qboolean		centerPrintAnnouncerMode;
	char			centerPrintAnnouncerText[512];
	char			centerPrintAnnouncerText2[512];

	specName_t		specOnScreenNames[MAX_CLIENTS];

	// jaquboss - client's logging
	fileHandle_t	logFile;

	qboolean		xpOrder;	// changed xp order?

	// IlDuca - CG_DrawDanger infos to draw arty in a good way
	// Time when the arty icon must finish
	int				artyTime;
	// Position of the arty
	vec3_t			artyPosition;
} cg_t;
...
typedef struct {
	...
	qhandle_t	resuscitateShader;
	qhandle_t	poisonedIcon;
	...
	qhandle_t	reviveActivateHintShader;
	...
	qhandle_t	blackmask; // jaquboss - etpro icons support
	...
	// qhandle_t	viewFlashBlood; // IRATA: unused
	...
	// qhandle_t	dirtParticle3Shader; // IRATA: not init/defined/in path

	qhandle_t	genericConstructionShader;
	qhandle_t	alliedUniformShader;
	qhandle_t	axisUniformShader;

	qhandle_t	specGlowShader[2];

	// special obituaries
	qhandle_t	fallObituary;
	qhandle_t	crushObituary;
	qhandle_t	drownObituary;
	qhandle_t	artyObituary;
	qhandle_t	airstrikeObituary;
	qhandle_t	poisonObituary;
	qhandle_t	goombaObituary;
	qhandle_t	shoveObituary;
	...
	sfxHandle_t	PrivateMessageBeep;

	// IRATA: precache sfxHandle_t for item pick up sound
	// remove this if we want to store sfxHandle_t in bgitemlist
	// (which is a big waste of memory -server side unused - and trapRegister calls)
	sfxHandle_t itemPickupAmmo;
	sfxHandle_t itemPickupHealth;
	sfxHandle_t itemPickupHealthSmall;
	sfxHandle_t itemPickupHot;
	sfxHandle_t itemPickupCold;

	sfxHandle_t itemPickupWeapon;
	sfxHandle_t itemPickupHelmet;
	sfxHandle_t itemPickupFlakjacket;
	sfxHandle_t itemPickupUniform;
	// end precache for item pick up sound
	...
	qhandle_t	hMountedMG42Base;
	qhandle_t	hMountedMG42Nest;
	qhandle_t	hMountedMG42;
	qhandle_t	hMountedBrowning;
	qhandle_t	hMountedFPMG42;
	qhandle_t	hMountedFPBrowning;

    qhandle_t   radioSound[2];
	...
	qhandle_t		limboClassButton2Wedge_on[NUM_SKILL_LEVELS-1];
	qhandle_t		limboClassButton2Wedge_off[NUM_SKILL_LEVELS-1];
	...
	sfxHandle_t		goombaSound;

	qhandle_t		hudFlakJacket[2];
	qhandle_t		hudHelmet[2];
	qhandle_t		hudAdrenaline;
	qhandle_t		hudBinoculars;
	qhandle_t		hudMorale[2];			// Morale Icon (axis/allies)
	qhandle_t		disguiseShader;
	qhandle_t		hudCaduceus;
	qhandle_t		hudEngineer;

	qhandle_t		auraCaduceus;
	qhandle_t		auraEngineer;
	qhandle_t		auraMorale[2];

	qhandle_t		tv_grain;

	qhandle_t		pistolHintShader[2];		// for execution style pistol shots (one for each team)

	qhandle_t		adrenalineEyesSkin;

	sfxHandle_t		knifeThrow;
	qhandle_t		nadeDanger[2];
	qhandle_t		countryFlags; //mcwf GeoIP
	qhandle_t		artyDanger;

	// IRATA: objective icons
	qhandle_t		gerusaFlag;
	qhandle_t		gerFlag;
	qhandle_t		usaFlag;

	// IRATA: intermission flags
	qhandle_t		flagshaderAxis;
	qhandle_t		nameshaderAxis;
	qhandle_t		flagshaderAllies;
...
#define JP_EXTRA_STRINGS 4	// core: replaced Darwin & Goomba with best-spree + bounty..
							// Also added a new line for hit-regions..

typedef struct {
	char strWS[WS_MAX][MAX_STRING_TOKENS];
	char strExtra[JP_EXTRA_STRINGS][MAX_STRING_TOKENS];
...
//	char				redTeam[MAX_QPATH];		// A team - IRATA: both unused
//	char				blueTeam[MAX_QPATH];	// B team
	...
	qhandle_t			gameModelSkins[MAX_MODELS];	//MAX_CS_SKINS
	bg_character_t		*gameCharacters[MAX_CHARACTERS];
	sfxHandle_t			gameSounds[MAX_SOUNDS];
	char				gameSoundNames[MAX_SOUNDS][MAX_QPATH];
	char				gameShaderstate[MAX_INFO_STRING];

	sfxHandle_t			cachedSounds[GAMESOUND_MAX]; // basically NQ sounds ... see also cgs.media
	qhandle_t			cachedModels[GAMEMODEL_MAX];
	...
	char				currentCampaign[MAX_CVAR_VALUE_STRING];
	...
	qboolean			mouseModeOn;		//	jet Pilot - are we in mouse pointer mode?
	...
	// core: display Mission Time Left during debriefing..
	int					dbMissionTimeMsec;
	...
	char				skillMedic[MAX_CVAR_VALUE_STRING];
	char				skillEngineer[MAX_CVAR_VALUE_STRING];
	char				skillFieldOps[MAX_CVAR_VALUE_STRING];
	char				skillCovertOps[MAX_CVAR_VALUE_STRING];
	char				skillBattleSense[MAX_CVAR_VALUE_STRING];
	char				skillLightWeapons[MAX_CVAR_VALUE_STRING];
	// g_spectator
	int					spectator;
	// g_realism
	int					realism;
	...
	// core: g_csMethod
	int					csMethod;
	int					csMethod_refresh;		// if == 1, check request new CS-data from server..
	int					csMethod_request;		// if == 1, do the actual request..
	int					csMethod_requesting;	// if != 0, the time the request was made..
	int					csMethod_total;			// the total number of strings to receive..
	int					csMethod_received;		// the number of strings received so far..
	int					csMethod_Models;		// cs data
	int					csMethod_Shaders;		// cs data
	int					csMethod_Skins;			// cs data
	int					csMethod_Characters;	// cs data
	int					csMethod_Sounds;		// cs data
	int					csMethod_Shaderstate;	// cs data
...
// extern	vmCvar_t		cg_letterbox;	//----(SA)	added IRATA_ removed
...
#endif
// extern	vmCvar_t 		cg_forceModel; // IRATA: not used
...
extern	vmCvar_t		cg_smokeparticles;
extern	vmCvar_t		cg_trailparticles;
...
extern  vmCvar_t		cg_drawHUDHead;
extern	vmCvar_t		cg_drawObjIcons;
...
extern	vmCvar_t		cg_markDistance;	// impacts beyond this range won't leave bullet marks
extern	vmCvar_t		cg_drawMuzzleFlash;
extern	vmCvar_t		cg_drawTracers;
extern	vmCvar_t		hud_powerupSize;
...
extern  vmCvar_t		hud_drawAltHUD;
...
// redeye - filter out popup messages
extern vmCvar_t			cg_popupMessageFilter;
extern vmCvar_t			cg_locations;
extern vmCvar_t			cg_fixedFTeamSize;
...
extern vmCvar_t			cg_whizzSounds;
...
#define SMALLFONT_POPUPS		0x02
#define SMALLFONT_MISSIONPOPUPS	0x04
#define SMALLFONT_LIMBO			0x08
...
// qboolean CG_BackupProfile(void); // IRATA: unused, ETPub leftover
...
//--- core: widescreen monitor support
...
void CG_DrawMicroStringColor( int x, int y, const char *s, vec4_t color );
// JOSEPH 4-25-00
...
void CG_PrintAnnouncement( const char *str, int y, int charWidth, int priority);		// jet Pilot
...
void CG_PredictLean(centity_t *cent, vec3_t	torsoAngles, vec3_t	headAngles,int viewHeight); // jaquboss
void CG_Jet( centity_t *cent);
// Rafael particles
...
qboolean QDECL CG_RW_ParseError( int handle, char *format, ... );
...
void CG_FireWeapon( centity_t *cent, int event, qboolean arty );	//----(SA)	modified.
...
void CG_RailTrail( vec3_t color, vec3_t start, vec3_t end, int type, int index );	//----(SA)	added 'type'
...
// void	CG_AddParticleShrapnel (localEntity_t *le); // IRATA: not implemented
...
// void	CG_SnowLink (centity_t *cent, qboolean particleOn); // IRATA: unused
...
void CG_PlayItemPickupSound(gitem_t	*item, int entityNum, qboolean checkSound);
...
#endif
char *CG_getBindKeyName(const char *cmd, char *buf, int len);	// core: used in cg_consolecmds.c
...
// void trap_PumpEventLoop( void ); // IRATA: obsolete/unused - see GPL
...
// RF
// void trap_SendMoveSpeedsToGame( int entnum, char *movespeeds ); // IRATA: not implemented
...
void CG_DrawFireTeamOverlay2( rectDef_t* rect );		// jet Pilot - declaration
...
int 				CG_LimboPanel_TeamCount2			( int team );
...
void CG_DrawPlayerHead( rectDef_t *rect, bg_character_t* character, bg_character_t* headcharacter, float yaw, float pitch, qboolean drawHat, hudHeadAnimNumber_t animation, qhandle_t painSkin, clientInfo_t *ci, qboolean spectator, int team, int cls, int disguise );
...
void CG_LoadPanel_LoadingBarText( panel_button_t* button ); // jaquboss draw what the game is loading
...
extern vec4_t	HUD_Progress;
extern float	HUD_Alpha;
...
#define POPUP_FILTER_OBJECTIVE		0x0001	// 1	- Filter out objective message(planted at ..., capture flags, ...)
#define POPUP_FILTER_PICKUPS		0x0002	// 2	- Filter out pickup messages(Picked up a health/ammo packs, weapons)
#define POPUP_FILTER_TEAMJOIN		0x0004	// 4	- Filter out team joining messages
#define POPUP_FILTER_CONNECT		0x0008	// 8	- Filter out player connect/disconnect messages
#define POPUP_FILTER_MAPINFO		0x0010	// 16	- Filter out any map script messages(command post constructed, tank damaged, ...)
#define POPUP_FILTER_ENEMYKILLS		0x0020	// 32	- Filter out all obituaries caused by the enemy team
#define POPUP_FILTER_TEAMKILLS		0x0040	// 64	- Filter out all obituaries caused by the your team
#define POPUP_FILTER_SELFKILLS		0x0080	// 128	- Filter out all obituaries of self kills(of all players)
#define POPUP_FILTER_OWNKILLS		0x0100	// 256	- filter out death/kill messages of yourself
#define POPUP_FILTER_CONSOLEKILLS	0x0200	// 512	- Filter out death/kill messages to console
...
// core: goatsound flags
#define SOUND_KNIFE_FRONT			0x01
#define SOUND_KNIFE_BACK			0x02
#define SOUND_KNIFE_THROW			0x04
...
// core: cg_drawCrosshairNames flags
#define CROSSHAIRNAMES_ENABLE		0x01	// for clarity..
#define CROSSHAIRNAMES_WHITENAMES	0x01	// for convenience..
#define CROSSHAIRNAMES_COLORNAMES	0x02
#define CROSSHAIRNAMES_RANKS		0x04

// core: cg_announcer flags
#define ANNOUNCER_DISABLED			0x00    // 0
#define ANNOUNCER_ENABLED			0x01	// 1 if not set, all announces are disabled (no sound, no text)
#define ANNOUNCER_NO_SPREES			0x02	// 2 disable killingspree/multikill/firstblood & firstheadshot
#define ANNOUNCER_OWN_KILLS_ONLY	0x04	// 4 only announce kills made by yourself (or the one you're observing)
#define ANNOUNCER_NO_CP				0x08	// 8 no centerprint message
#define ANNOUNCER_NO_SOUND			0x10	// 16 no voice sound
//#define ANNOUNCER_NO_KILLS			0x20	// 32 no normal kill announces		cs: disabled for now

// cg_drawKillSpree flags
#define DRAWKILLSPREE_CURRENT  		1 // current spree
#define DRAWKILLSPREE_KILLS 		2 // kills
#define DRAWKILLSPREE_BOUNTY 		4 // bounty
#define DRAWKILLSPREE_DEATHS 		8 // deaths


//----------------------------
// core: smoother player-icons movement on the commandmap..
// used in cg_commandmap.c
#define MAX_PLAYERPATH		2
...
typedef struct {
	int					count;						// total entries stored/valid..
	playerPathPos_t		current[MAX_PLAYERPATH];
} playerPath_t;
//----------------------------



// core: obituary information record
// used in cg_event.c
typedef struct {
	qboolean		overrideIcon;
	char			*message;
	char			*message2;
	qhandle_t		icon;
} obituaries_record_t;
...
// core: reset weapon restrictions data..
// used in cg_servercmds.c
void C_ResetWeaponRestrictions( void );
